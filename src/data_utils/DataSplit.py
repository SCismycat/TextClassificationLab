...
from src.data_utils.TextProcUtil import txt_write, txt_read
...
def data_split_train_val_label(path_org_data, path_save_dir, count_num=500000, use_shuffle=True):
    ...
    # 是否扰乱
    if use_shuffle:
        trains = txt_read(path_save_dir + "train.csv")
        valids = txt_read(path_save_dir + "valid.csv")
...
if __name__ == '__main__':
    from src.conf.path_config import path_root
    filepath = path_root + "/data/train_data.csv"  # 原始语料
    data_split_train_val_label(path_org_data=filepath,
                               path_save_dir=path_root + "/data/xinhuanet/",
                               count_num=500000,
                               use_shuffle=True)
