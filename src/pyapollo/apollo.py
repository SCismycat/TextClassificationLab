...
class ApolloClient(object):
    def __init__(self, app_id, config_server_url='http://localhost:8080', cluster='default', timeout=35, ip=None):
...
# apollo_id=str(os.getenv('APP_ID'))
apollo_id = 201947722
print("app_id", apollo_id)

apollo_url = str(os.getenv('APOLLO'))
# apollo_url = "http://192.168.4.31:8083"
print("apollo", apollo_url)

if not apollo_url:
    print("apollo环境变量读取出错")
if apollo_url.startswith("http://"):
    print("apollo url:"+apollo_url)
else:

    apollo_url = apollo_url.split("-Dapollo.meta=")[1]
    print("切割后的apollo",str(apollo_url))
client = ApolloClient(app_id=apollo_id, config_server_url=apollo_url)
print("开始启动apollo")
client.start()
print("启动结束！")


def get_config(key, namespace):
    back = client.get_value(key=key, namespace=namespace)
    return back
...
#     back = client.get_value(key='redis_ip', namespace='kgchannel')
#     print(back)
#     client.stop()
